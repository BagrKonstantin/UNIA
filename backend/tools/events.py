...
def get_upcoming_events(start_date: Optional[str] = None, end_date: Optional[str] = None) -> str:
    """
    Use this tool whenever a student asks about upcoming one-time events, parties, or special gatherings.
    (For repeating classes/workshops, use the get_workshops tool instead).
    ...
    For example, if the user asks for events "next week" or "next Friday", calculate the exact start and end dates based on today's date and pass them.
